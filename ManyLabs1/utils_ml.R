# edit on Oct 8, 2024:
# --- added T-invariant analysis 
# --- added collinearity removal to entropy balancing
...
compute_KL <- function(df1, df2, alg = 'logistic',
                       covariates, 
                       analysis_formula, 
                       treatment_variable){
  ...
  if (alg == 'grf'){
    mdl = regression_forest(rbind(df1_KL, df2_KL),
                            c(rep(0, nrow(df1_KL)), rep(1, nrow(df2_KL))), num.threads = 1)
  ...
  KL_est = as.numeric(mean(drs1 * log(drs1)))
  
  return(KL_est)
  
}

grad.kl <- function(ifl, lam, delta){
  return( - mean(exp(-ifl/lam-1)) - delta - mean(ifl * exp(-ifl / lam -1)) / lam )
}

obj.kl <- function(ifl, lam, delta){
  return( - lam  * (mean(exp(- ifl/lam - 1)) + delta) )
}


KL_single <- function(ifl, delta){
  # lambda.list = seq(0.0001, 10, by = 0.001)
  # 
  # obj.list = c()
  # for (i in 1:length(lambda.list)){
  #   obj.list = c(obj.list, - lambda.list[i] * (mean(exp(- ifl/lambda.list[i] - 1)) + delta))
  # }
  lam.left = 0.00001
  while (obj.kl(ifl, lam.left, delta) %in% c(Inf, - Inf)){
    lam.left = lam.left * 2
  }
  lam.right = 10
  while (grad.kl(ifl, lam.right, delta) >0){
    lam.right = lam.right * 2
  }
  lam.mid = (lam.left + lam.right) / 2
  grad.left = grad.kl(ifl, lam.left, delta)
  grad.right = grad.kl(ifl, lam.right, delta)
  grad.mid = grad.kl(ifl, lam.mid, delta)
  ...
  lam.final = lam.mid
  obj.final = obj.kl(ifl, lam.final, delta)
...
compute_KL_bound <- function(df1, df2, delta, 
  ...
  ifl = df1$infl
  kl.res = KL_single(ifl, delta)
  est.lower = kl.res$opt.obj
  kl.res.upp = KL_single(- ifl, delta)
...
###  compute KL divergence across two sites  ###
# generalize from df1 to df2 to be consistent with other estimators
# compute KL divergence E_P[Q/P * log(Q/P)] where P is the weighted df1 distribution
###################################################
...
#   return( - mean(w*exp(-ifl/lam-1)) - delta - mean(w*ifl * exp(-ifl / lam -1)) / lam )
# }
# 
# # lower bound of sample mean (weighted) under KL <= delta
# obj.kl.w <- function(ifl, lam, delta, w){
#   return( - lam  * (mean(w*exp(- ifl/lam - 1)) + delta) )
# }
...
KL_single.w <- function(ifl, delta, w){
  ...
  lam.right = max(abs(ifl))
  # print(grad.kl.w(ifl, lam.right, delta, w))
...
###################################################
### Utils functions for separately estimating treated and control groups
###################################################

compute_decomp_grouped <- function(df1, df2, K=5, algorithm = 'grf', estimator = 'ebal',
                                   covariates = NULL, 
                                   treatment_variable = NULL, 
                                   response_variable = NULL){
  
  df1.treated = df1 %>% filter(!!sym(treatment_variable) == 1) 
  df1.control = df1 %>% filter(!!sym(treatment_variable) == 0)
  
  df2.treated = df2 %>% filter(!!sym(treatment_variable) == 1) 
  df2.control = df2 %>% filter(!!sym(treatment_variable) == 0)
  
  analysis_formula = paste(response_variable, "~ 1")
  
  res.treated = compute_decomp(df1.treated, df2.treated, covariates, 
                               K=K, algorithm=algorithm, estimator=estimator,
                               analysis_formula=analysis_formula,
                               treatment_variable = "(Intercept)")
  
  res.control = compute_decomp(df1.control, df2.control, covariates, 
                               K=K, algorithm=algorithm, estimator=estimator,
                               analysis_formula=analysis_formula,
                               treatment_variable = "(Intercept)")
  
  return(list("treated" = res.treated, "control" = res.control))
}
...
compute_decomp <- function(df1, df2, K=5, algorithm = 'grf', estimator = 'ebal',
                           covariates = NULL,
                           analysis_formula = NULL, 
                           treatment_variable = NULL, 
                           focal_variable = NULL,
                           covariate_formula = NULL, eps_w=0.05){
  ...
  if (estimator == 'double'){ 
    ...
    for (k in 1:K){
      if (length(covariates) == 1){
        ...
        if (algorithm == 'grf'){
          c.phi = regression_forest(data.frame("x" = df.phi$x), df1$infl[idx.1.comp[[k]]], num.threads = 1)
          c.w = regression_forest(df.w$x, df.w$y, num.threads = 1) 
        ...
        if (algorithm == 'grf'){
          c.phi = regression_forest(df1[idx.1.comp[[k]],covariates], df1$infl[idx.1.comp[[k]]], num.threads = 1) 
          ...
          c.w = regression_forest(df.w[,covariates], df.w$y, num.threads = 1) 
  ...
  # theta.x = mean(phi.pred.2) - mean(w.pred * phi.pred.1) + mean(w.pred * df1$infl)
  ...
  if (estimator == 'ebal'){
    ...
    theta1 = as.numeric(fit$decomp['Theta_org'])
    theta2 = as.numeric(fit$decomp['Theta_rep'])
    ...
    for (k in 1:K){  
      if (length(covariates)==1){
        ...
        if (algorithm == 'grf'){
          c.phi = regression_forest(data.frame("x" = df.phi$x), df.phi$infl, num.threads = 1)  
          phi.pred.1 = c(phi.pred.1, predict(c.phi, data.frame("x"=df1[idx.1[[k]], covariates]))$predictions) 
        }else if (algorithm == 'loess'){
          c.phi = suppressWarnings(loess(infl~., data = df.phi, span=1)) 
          phi.pred.1 = c(phi.pred.1, predict(c.phi, df1[idx.1[[k]],covariates])) 
...
###################################################
# the same as compute_decomp other than centering influence function
###################################################

compute_decomp_center <- function(df1, df2, K=5, algorithm = 'grf', estimator = 'ebal',
                           covariates = NULL,
                           analysis_formula = NULL, 
                           treatment_variable = NULL, 
                           focal_variable = NULL,
  ...
  if (estimator == 'double'){ 
    ...
    for (k in 1:K){
      if (length(covariates) == 1){
        ...
        if (algorithm == 'grf'){
          c.phi = regression_forest(data.frame("x" = df.phi$x), df1$infl_centered[idx.1.comp[[k]]], num.threads = 1)
          c.w = regression_forest(df.w$x, df.w$y, num.threads = 1) 
          phi.pred.1 = c(phi.pred.1, predict(c.phi, data.frame("x" = df1[idx.1[[k]], covariates]))$predictions)
          phi.pred.2 = c(phi.pred.2, predict(c.phi, data.frame("x" = df2[idx.2[[k]], covariates]))$predictions)
        ...
        if (algorithm == 'grf'){
          c.phi = regression_forest(df1[idx.1.comp[[k]],covariates], df1$infl_centered[idx.1.comp[[k]]], num.threads = 1) 
          c.w = regression_forest(df.w[,covariates], df.w$y, num.threads = 1) 
          phi.pred.1 = c(phi.pred.1, predict(c.phi, df1[idx.1[[k]], covariates])$predictions)
          phi.pred.2 = c(phi.pred.2, predict(c.phi, df2[idx.2[[k]], covariates])$predictions)
    ...
    w.pred = w.pred * nrow(df1) / nrow(df2)
    theta.x = mean(phi.pred.2) - mean((phi.pred.1+ mean(df1$infl)) * w.pred) + mean(w.pred * df1$infl)
  ...
  cal.lm = lm(y~., data = cal.df)
  phi.pred.1.cal = predict(cal.lm) # linear calibration of predicted influence function
  s.yx.squared = min(mean((df1$infl - phi.pred.1.cal)^2), sd(df1$infl)^2)
  s.x.squared = mean(phi.pred.1.cal * (2*df1$infl - phi.pred.1.cal)) - mean(df1$infl)^2
  s.yx = sqrt(s.yx.squared)
  s.x = sqrt(s.x.squared) 
...
compute_basic <- function(df1, df2, K=5,  
  ...
  X1 = data.frame(df1[,covariates])
  X2 = data.frame(df2[,covariates])
  X1 = as.matrix(X1)
  X2 = as.matrix(X2)
...
compute_stable <- function(df1, df2, K=5, algorithm = 'grf', estimator = 'ebal',
  ...
  if (estimator == 'double'){ 
    ...
    # 
    # if (center){
    #   theta.x = mean(phi.pred.2) - mean((phi.pred.1+ mean(df1$infl)) * w.pred) + mean(w.pred * df1$infl_centered) 
    # }else{
    theta.x = mean(phi.pred.2) - mean(phi.pred.1 * w.pred) + mean(w.pred * df1$infl)
    # }
  ...
  # compute sensitivity measures / variances
...
transfer_dr <- function(df1, df2, K=5, algorithm = 'grf', 
  ...
  df1$infl = infl.1[,treatment_variable]
  df2$infl = infl.2[,treatment_variable]
  df1$infl_centered = infl.1[,treatment_variable] - mean(infl.1[,treatment_variable])
  df2$infl_centered = infl.2[,treatment_variable] - mean(infl.2[,treatment_variable])
  ...
  # use different point estimates for transfer
  if (method == 'ebal'){
    df1$myid = 1:nrow(df1)
    df2$myid = 1:nrow(df2)
...
###################################################
### Transfer without adjusting covariate shift ####
...
transfer_plain <- function(df1, df2, 
  ...
  # theta2 = mean(df2$infl)
  # # transfer variance  
  # trans.var = sd(df1$infl)^2 * (1/nrow(df1) + 1/nrow(df2))
  
  # == new version Oct 1 2024, same as utils_decomp_v2 for pipeline
  infl.1 = t(solve(t(X.1) %*% X.1 / nrow(X.1)) %*%  t(diag(y.1) %*% X.1))
  infl.2 = t(solve(t(X.2) %*% X.2 / nrow(X.2)) %*%  t(diag(y.2) %*% X.2))
  resid.1 = (as.numeric(y.1) - as.numeric(X.1 %*% as.matrix(as.numeric(colMeans(infl.1)), ncol=1)))
  resid.2 = (as.numeric(y.2) - as.numeric(X.2 %*% as.matrix(as.numeric(colMeans(infl.2)), ncol=1)))
  ...
  # sd.1 = diag(sqrt( cov(t(solve(t(X.1) %*% X.1 / nrow(X.1)) %*% t(X.1) %*% diag(resid.1))) ) /sqrt(nrow(df1)))[treatment_variable]
  sd.1 = diag(sqrt(cov(c.infl.1)))[treatment_variable]
  
  theta1 = mean(infl.1[,treatment_variable]) 
  theta2 = mean(infl.2[,treatment_variable])
  # transfer variance  
  # trans.var = sd.1^2 * (1 + nrow(df1) /nrow(df2))
...
###################################################
### Transfer with diff-in-mean ratio of influence function vs covariates ####
# transfer from df1 to df2 
###################################################

transfer_ratio <- function(df1, df2, covariates,
                           analysis_formula = NULL, 
                           treatment_variable = NULL){
  ...
  theta2 = mean(df2$infl)
  # transfer variance  
  y.shift = (mean(df2$infl) - mean(df1$infl)) / sd(df1$infl)  
  y.sens = sd(df1$infl)  
...
decomposition_helper = function(
  ...
  # remove collinearity 
  pca0 = prcomp(X2)
  imp0 = summary(pca0)$importance[2,]
  imp_thd=0.001
  X2 = matrix(pca0$x[, imp0>imp_thd], ncol = sum(imp0>imp_thd))
  X1 = matrix(predict(pca0, X1)[, imp0>imp_thd], ncol = sum(imp0>imp_thd))
...
# 
# 
# compute_decomp_stable <- function(df1, df2, K=5, algorithm = 'grf', 
#                            covariates = NULL,
#                            analysis_formula = NULL, 
#                            treatment_variable = NULL, 
#                            focal_variable = NULL,
...
#   
#    
#   # compute parameter shift components 
#   phi.pred.1 = c()
#   phi.pred.2 = c()
#   w.pred = c()
#   for (k in 1:K){
...
#         c.phi = regression_forest(data.frame("x" = df.phi$x), df1$infl[idx.1.comp[[k]]])
#         # c.w = regression_forest(df.w$x, df.w$y)
#         # c.w.lin = glm(y~., data = data.frame("y" = df.w$y,
#         #                                      "x" = predict(c.w, df.w[,covariates])$predictions),)
#         phi.pred.1 = c(phi.pred.1, predict(c.phi, data.frame("x" = df1[idx.1[[k]], covariates]))$predictions)
#         phi.pred.2 = c(phi.pred.2, predict(c.phi, data.frame("x" = df2[idx.2[[k]], covariates]))$predictions)
#         # w.pred = c(w.pred, predict(c.w, data.frame("x" = df1[idx.1[[k]],covariates]))$predictions) 
#       }else if (algorithm == 'loess'){
...
#   cal.lm = lm(y~., data = cal.df)
#   phi.pred.1.cal = predict(cal.lm)
#   s.yx.squared = sd(df1$infl)^2 - sd(phi.pred.1.cal)^2
#   s.x.squared = sd(phi.pred.1.cal)^2
#   s.yx = sqrt(s.yx.squared)
#   s.x = sqrt(s.x.squared) 
#   
#   # compute shift strength
...
#   x.shift = delta.x / s.x
#   
#   return(list("delta_total" = delta.yx + delta.x, 
#               "delta_yx" = delta.yx, "delta_x" = delta.x, 
#               "cond_sensitivity" = s.yx, "x_sensitivity" = s.x, 
#               "cond_shift" = cond.shift, "x_shift" = x.shift))
# }
