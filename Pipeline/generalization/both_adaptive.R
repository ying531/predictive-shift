...
load("../Rdata/results_K5_stable_filtered_centered.RData")
load("../Rdata/results_weighted.RData")
load("../Rdata/results_plain.RData")
load("../Rdata/results_ratio_no_weight.RData") 
load("../Rdata/cond_KL_delta_new.RData") 


analysis.studies = all.studies %>% 
  left_join(ratio_no_w.studies, by = c("data", "study"), suffix = c("", "._now")) %>% 
  left_join(all.trans, by = c("data", "study"), suffix = c("", "._trans")) %>% 
  left_join(all.plain, by = c("data", "study"), suffix = c("", "._plain")) %>% 
  drop_na(any_of(c('stab_x_shift', 'delta_yx')))

analysis.global_qts = analysis.studies %>%  
  group_by(alg) %>% 
...
analysis.study_qts = analysis.studies %>%  
  group_by(data, alg) %>% 
...
merge.res = analysis.studies %>% 
  left_join(analysis.global_qts, by = 'alg') %>% 
  left_join(analysis.study_qts, by = c('data', 'alg')) 
...
all_times = rep(0, length(all_sites))
for (data_id in 1:10){
  data_all = clean_vars(read.csv(paste(data_path, data_list[data_id], sep = '')),
                        turn_numerics = turn_numeric_list[[data_id]], 
                        othername = (data_id %in% c(5,7,8))) 
  id_sites = unique(data_all$datacollection) 
  all_times[(1:29)[all_sites[1:29] %in% id_sites]] = all_times[(1:29)[all_sites[1:29] %in% id_sites]] + 1
}
freq.mat = data.frame("id" = 1:29, "sites" = all_sites, "occurance" = all_times)
...
exist.site.id = all_sites[site.order[1:(step.id*3)]]
new.site.id = all_sites[site.order[(step.id*3+1): 29]]

exist.study.id = study.order[1:step.id]
new.study.id = study.order[(step.id+1):10]
 
###===== generate KL bounds  =====###  

KL_max = max(KL.delta.all %>% filter(study1 %in% exist.site.id, 
                               study2 %in% exist.site.id, 
                               data %in% exist.study.id) %>% select(KL), na.rm=TRUE) 
KL_qt = quantile(KL.delta.all %>% filter(study1 %in% exist.site.id, 
                                   study2 %in% exist.site.id, 
                                   data %in% exist.study.id) %>% select(KL), 0.95, na.rm=TRUE)

all_res_max = data.frame()
all_res_qt = data.frame()

for (data_id in new.study.id){
  data_all = clean_vars(read.csv(paste(data_path, data_list[data_id], sep = '')),
                        turn_numerics = turn_numeric_list[[data_id]], 
                        othername = (data_id %in% c(5,7,8)))
  
  # generate all pairs of sites
  all_sites = intersect(unique(data_all$datacollection), new.site.id)
  ...
  for (ii in 1:nrow(all_grid)){  
    ...
    if (!is.null(ii.res.qt)){ 
      ...
      ii.res.qt$study1 = all_sites[all_grid[ii,1]]
      ii.res.qt$study2 = all_sites[all_grid[ii,2]]
...
write.csv(all_res_max, paste0("../output/both_adaptive/res_KLmax_", step.id, "_seed_", seed, ".csv"))
write.csv(all_res_qt, paste0("../output/both_adaptive/res_KLqt_", step.id, "_seed_", seed, ".csv"))

for (alg_name in c("double", "ebal")){
  
  qt.id = analysis.studies %>% filter(study1 %in% exist.site.id, 
                                      study2 %in% exist.site.id, 
                                      data %in% exist.study.id, alg == alg_name) %>% 
    summarize( 
      qt_upp_stab = quantile(cond_shift / stab_x_shift, 1-alpha/2),
      qt_low_stab = quantile(cond_shift / stab_x_shift, alpha/2)  
    ) 
  
  PI.remain = merge.res %>% filter(study1 %in% new.site.id, 
                                   study2 %in% new.site.id, 
                                   data %in% new.study.id, alg==alg_name) %>% 
    mutate( 
  ...
  )  %>% mutate(source_nums = step.id, alg = alg_name, seed = seed)
  
  study.res = rbind(study.res, summary.remain)
}  


write.csv(study.res, paste0("../output/both_adaptive/sum_", step.id, "_seed_", seed, ".csv"))
